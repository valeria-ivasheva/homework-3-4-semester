...
type TestClass () =
    ...
    [<TestMethodAttribute>]
    member this.``Power 2^(-3) expected InputException`` () = 
        (fun () -> Func.power -3 2 |> ignore) |> should  throw typeof<Func.InputException>

    [<TestMethodAttribute>]
    member this.``Power 2^3 should equal 8`` () = 
        Func.power 3 2 |> should equal 8

    [<TestMethodAttribute>]
    member this.``Power 133^0 should equal 1`` () = 
        Func.power 0 133 |> should equal 1
